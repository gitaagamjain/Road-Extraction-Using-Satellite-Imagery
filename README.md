...
Due to file size limits, the dataset is not included in this repository. It must be downloaded separately from Kaggle.
...
An initial training run on 400 images resulted in heavy overfitting. The final model pipeline implements three standard regularization techniques:

Dataset Expansion: Increased the training subset to 2,000 images.
...
Scale: This model was trained on a restricted subset of the total dataset due to compute limits. It is an MVP, not a production-ready model.

Edge Cases: The model may still struggle with complex intersections, dirt roads, or roads heavily obscured by dense tree canopies.
...
Set the runtime hardware accelerator to T4 GPU.
